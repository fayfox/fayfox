...
class Menu extends Model{
	...
	public function buildIndex($parent = 0, $start_num = 1){
		Tree::model()->buildIndex('fayfox\models\tables\Menus', $parent, $start_num);
	}
	...
	public function create($parent, $sort = 100, $data = array()){
		return Tree::model()->create('fayfox\models\tables\Menus', $parent, $sort, $data);
	}
	...
	public function remove($id){
		return Tree::model()->remove('fayfox\models\tables\Menus', $id);
	}
	...
	public function removeAll($id){
		return Tree::model()->removeAll('fayfox\models\tables\Menus', $id);
	}
	...
	public function update($id, $data, $sort = null, $parent = null){
		return Tree::model()->edit('fayfox\models\tables\Menus', $id, $data, $sort, $parent);
	}
	...
	public function sort($id, $sort){
		Tree::model()->sort('fayfox\models\tables\Menus', $id, $sort);
	}
	...
	public function getTree($alias = null, $real_link = true){
		if($alias === null){
			return Tree::model()->getTree('fayfox\models\tables\Menus');
		}else{
			$node = $this->getByAlias($alias, 'id');
			if($node){
				$menu = Tree::model()->getTree('fayfox\models\tables\Menus', $node['id']);
				if($real_link){
